...
window.limparValorPlanilha = val => { if(!val) return 0; if(typeof val==='number') return val; let str=String(val).trim().replace(/R\$\s?/gi,'').trim(); if(str.includes(',')&&str.includes('.')){str=str.replace(/\./g,'').replace(',','.');}else if(str.includes(',')){str=str.replace(',','.');} let num=parseFloat(str); return isNaN(num)?0:num; };
window.formatarDoBanco = val => window.fmt(window.limparValorPlanilha(val));
window.formatarPreco = function(val){ if(!val) return ""; let v=String(val).trim(); if(v.toUpperCase().includes("R$")) return v; if(!isNaN(v.replace(',','.'))) return "R$ " + parseFloat(v.replace(',','.')).toFixed(2).replace('.',','); return "R$ "+v; };
...
window.tentarLogin = async function() {
    ...
    try {
        ...
        if (res.sucesso) {
            ...
            window.iniciarApp();
        } else { alert(res.erro || "Usuário ou senha incorretos."); window.$('input-senha').value = ""; }
...
window.iniciarApp = function(){
    window.$('login-screen').style.display="none"; window.$('app-core').style.display="block";
...
window.addLog = function(msg, tipo='info'){ window.$('terminal').innerHTML+=`<div class="log-line"><span class="log-time">[${new Date().toLocaleTimeString('pt-BR')}]</span><span class="log-${tipo}">${msg}</span></div>`; window.$('terminal').scrollTop=window.$('terminal').scrollHeight; };
window.mostrarAlerta = function(tit, txt, tipo){ window.$('modal-titulo-alert').innerText=tit; window.$('modal-texto-alert').innerText=txt; const ic=window.$('modal-icone-alert'), b=window.$('btn-modal-ok'); if(tipo==="success"){ic.innerText="✅";b.style.background="#10b981";b.style.boxShadow="0 6px 0 #059669";}else if(tipo==="warning"){ic.innerText="⚠️";b.style.background="#f59e0b";b.style.boxShadow="0 6px 0 #d97706";}else{ic.innerText="❌";b.style.background="#ef4444";b.style.boxShadow="0 6px 0 #b91c1c";} window.$('custom-modal').style.display="flex"; if("vibrate" in navigator) navigator.vibrate(50); };
...
window.abrirModalEditarVenda = function(l){ const v=window.arrayVendas.find(x=>x.linha===l); if(!v)return; window.$('edit-venda-linha').value=v.linha; window.$('edit-venda-data').value=v.data.includes('/')?`${v.data.split('/')[2].substring(0,4)}-${v.data.split('/')[1].padStart(2,'0')}-${v.data.split('/')[0].padStart(2,'0')}`:""; window.$('edit-venda-status').value=v.status; window.$('edit-venda-cliente').value=v.cliente; window.$('edit-venda-produto').value=v.produto; window.$('edit-venda-plataforma').value=v.plataforma; window.$('edit-venda-qtd').value=v.qtd; window.$('edit-venda-valor').value=window.formatarDoBanco(v.valor_venda); window.$('custom-edit-venda-modal').style.display="flex"; };
window.excluirRegistroLocal = async function(aba, l, idBtn, fnFechar, fnRecarregar, apiBase=URL_ANALYTICS_CATALOGO){ const b=window.$(idBtn); b.disabled=true; b.innerText="⏳ Excluindo..."; try { const r=await fetch(apiBase,{method:"POST",body:JSON.stringify({acao:"excluir_registro",aba:aba,linha:l})}); const res=await r.json(); if(res.sucesso){window.addLog(`🗑️ Registro excluído.`, "success"); if(fnFechar)fnFechar(); if(fnRecarregar)fnRecarregar();}else throw new Error("Erro exclusão."); }catch(e){ apiBase===API_PRECIFICACAO?window.mostrarAlerta("Erro","Falha: "+e.message,"error"):alert("Falha: "+e.message); }finally{b.disabled=false;b.innerText="🗑️ Excluir";} };
...
window.renderizarPlataformas = function() {
    ...
    plats.forEach((p, idx) => {
        h += `<div class="plat-item"><div class="plat-info"><strong>${p.nome}</strong><span>Taxa: ${p.pct}% + R$ ${p.fixa.toFixed(2).replace('.',',')} fixa</span></div><button class="btn-editar" style="background:#fef2f2; border-color:#fecaca; color:#b91c1c;" onclick="excluirPlataforma(${idx})">🗑️</button></div>`;
...
// MÓDULO ÍMAS NFC
// ==========================================
window.carregarImas = async function() { window.$('loading-imas').style.display = 'block'; window.$('lista-imas-admin').innerHTML = ''; try { const r = await fetch(API_NFC + "?acao=listar"); const res = await r.json(); if (res.sucesso) { window.arrayImas = res.imas.reverse(); window.renderListaImas(window.arrayImas); } else throw new Error("Erro ao buscar imas."); } catch (e) { window.addLog(`Erro Ímãs: ${e.message}`, "error"); } finally { window.$('loading-imas').style.display = 'none'; } };
window.renderListaImas = function(arr) { const lE = window.$('lista-imas-admin'); if (arr.length === 0) { lE.innerHTML = "<p style='text-align:center;color:#999;padding:20px;'>Nenhum ímã cadastrado.</p>"; return; } let h = ""; arr.forEach(i => { const statusVisual = i.linkVideoYoutube ? `<span class="tag-ativo">🎬 Vídeo Ativo</span>` : `<span class="tag-pendente">⏳ Teaser</span>`; let dataFormatada = "Sem data alvo"; if(i.dataLiberacao) { let d = new Date(i.dataLiberacao); if(!isNaN(d)) dataFormatada = `${String(d.getDate()).padStart(2,'0')}/${String(d.getMonth()+1).padStart(2,'0')}/${d.getFullYear()} às ${String(d.getHours()).padStart(2,'0')}:${String(d.getMinutes()).padStart(2,'0')}`; } const fotoUrl = i.foto ? i.foto.split(',')[0].trim() : 'logo.png'; const linkNfc = `https://diario.vivainteligente.net/ima-nfc/?id=${i.id}`; h += `<div class="list-item"><img src="${fotoUrl}" class="list-img" onerror="this.src='logo.png'"><div class="list-info"><h4 class="list-nome">ID: ${i.id} - ${i.nomes}</h4><div class="list-detalhes">🗓️ Liberação: ${dataFormatada}</div><div style="margin-top:8px;">${statusVisual}</div></div><button class="btn-editar" style="margin-right:8px; background:#e0f2fe; border-color:#bae6fd; color:#0369a1;" onclick="copiarLinkNfc('${linkNfc}')" title="Copiar Link">🔗 Link</button><button class="btn-editar" onclick="abrirModalEditarIma(${i.linha})">✏️</button></div>`; }); lE.innerHTML = h; };
window.copiarLinkNfc = function(link) { navigator.clipboard.writeText(link).then(() => { window.mostrarAlerta("Copiado!", "O link do ímã foi copiado para sua área de transferência!", "success"); }).catch(() => { alert("Erro ao copiar o link."); }); };
...
window.fazerUploadRedundante = async function(fC, ik) { let urlOnion = ""; let urlImgBB = ""; try { const fd = new FormData(); fd.append("imagem", fC); const rp = await fetch(API_ONIONSYS, {method:"POST", headers:{"Authorization":`Bearer ${TOKEN_ONIONSYS}`, "x-tenant-id":"MiniMundo"}, body:fd}); const tx = await rp.text(); if (rp.ok) { const rs = JSON.parse(tx); urlOnion = (rs.arquivos && rs.arquivos.length > 0) ? rs.arquivos[0].url : (rs.url || rs.link || rs.URL || (rs.filename ? `https://api.onionsys.com.br/arquivos/catalogo/${rs.filename}` : "")); window.addLog("✅ Imagem salva no servidor Primário.", "success"); } } catch (e) { window.addLog("⚠️ Servidor primário falhou.", "warn"); } if (ik) { try { const fd2 = new FormData(); fd2.append("image", fC); const r2 = await fetch(`https://api.imgbb.com/1/upload?key=${ik}`, {method:"POST", body:fd2}); const d2 = await r2.json(); if (d2.success) { urlImgBB = d2.data.url; window.addLog("✅ Imagem salva no Backup (ImgBB).", "success"); } } catch (e) { window.addLog("⚠️ Servidor de backup falhou.", "warn"); } } if (!urlOnion && !urlImgBB) throw new Error("Ambos os servidores de imagem falharam."); let urlsFinais = []; if (urlOnion) urlsFinais.push(urlOnion); if (urlImgBB) urlsFinais.push(urlImgBB); return urlsFinais.join(","); };
window.cadastrarIma = async function() { const id = window.$('ima-id').value.trim(); const nomes = window.$('ima-nomes').value.trim(); const fFoto = window.$('ima-foto').files; const fProd = document.createElement('input'); fProd.type = 'file'; const video = window.$('ima-video').value.trim(); const dataOriginal = window.$('ima-data').value; const ik = window.$('imgbb-key').value.trim(); const inputProd = document.getElementById("ima-foto-produto"); if(!id || !nomes || fFoto.length === 0 || !inputProd.files[0]) return window.mostrarAlerta("Atenção", "Preencha ID, Nomes e as DUAS fotos (Cliente e Produto).", "warning"); const b = window.$('btn-salvar-ima'); b.disabled = true; b.innerText = "⏳ Gerando..."; try { window.addLog(`Subindo foto do cliente...`, "info"); const urlsCliente = await window.fazerUploadRedundante(await window.comprimirImagem(fFoto[0], 600, 600, 0.8), ik); window.addLog(`Subindo foto do produto físico...`, "info"); const urlsProduto = await window.fazerUploadRedundante(await window.comprimirImagem(inputProd.files[0], 600, 600, 0.8), ik); const p = { acao: "salvar_ima", id: id, nomes: nomes, foto: urlsCliente, linkVideoYoutube: video, dataLiberacao: dataOriginal ? dataOriginal + ":00" : "", fotoProduto: urlsProduto }; const r = await fetch(API_NFC, { method: "POST", body: JSON.stringify(p) }); const res = await r.json(); if(res.sucesso) { window.mostrarAlerta("Sucesso!", "Ímã cadastrado com fotos personalizadas!", "success"); window.carregarImas(); } } catch (e) { window.addLog(`❌ ERRO: ${e.message}`, "error"); } finally { b.disabled = false; b.innerText = "✨ Gerar Ímã"; } };
window.abrirModalEditarIma = function(l) { const i = window.arrayImas.find(x => x.linha === l); if(!i) return; window.$('edit-ima-linha').value = i.linha; window.$('edit-ima-nomes').value = i.nomes; window.$('edit-ima-video').value = i.linkVideoYoutube; window.$('edit-ima-foto-antiga').value = i.foto; window.$('edit-ima-img-preview').src = i.foto ? i.foto.split(',')[0].trim() : "logo.png"; window.$('edit-ima-foto').value = ""; window.$('edit-ima-foto-prod-antiga').value = i.fotoProduto || ""; if(i.fotoProduto) { window.$('edit-ima-prod-preview').src = i.fotoProduto.split(',')[0].trim(); window.$('edit-ima-prod-preview').style.display = "block"; } else { window.$('edit-ima-prod-preview').style.display = "none"; } window.$('edit-ima-foto-prod').value = ""; if(i.dataLiberacao) { window.$('edit-ima-data').value = String(i.dataLiberacao).substring(0, 16); } else { window.$('edit-ima-data').value = ""; } window.$('modal-editar-ima').style.display = 'flex'; };
window.salvarEdicaoIma = async function() { const b = window.$('btn-salvar-edicao-ima'); b.disabled = true; b.innerText = "⏳ Atualizando..."; try { const ik = window.$('imgbb-key').value.trim(); const idAtual = window.arrayImas.find(x => x.linha == parseInt(window.$('edit-ima-linha').value)).id; let fotoFinal = window.$('edit-ima-foto-antiga').value; const novaFoto = window.$('edit-ima-foto').files; let fotoProdFinal = window.$('edit-ima-foto-prod-antiga').value; const novaFotoProd = window.$('edit-ima-foto-prod').files; if(novaFoto.length > 0) { window.addLog(`Atualizando foto do cliente...`, "info"); fotoFinal = await window.fazerUploadRedundante(await window.comprimirImagem(novaFoto[0], 600, 600, 0.8), ik); } if(novaFotoProd.length > 0) { window.addLog(`Atualizando foto do produto...`, "info"); fotoProdFinal = await window.fazerUploadRedundante(await window.comprimirImagem(novaFotoProd[0], 600, 600, 0.8), ik); } let dataLiberacaoStr = ""; if(window.$('edit-ima-data').value) { dataLiberacaoStr = window.$('edit-ima-data').value + ":00"; } const p = { acao: "atualizar_ima", linha: window.$('edit-ima-linha').value, id: idAtual, nomes: window.$('edit-ima-nomes').value, foto: fotoFinal, linkVideoYoutube: window.$('edit-ima-video').value, dataLiberacao: dataLiberacaoStr, fotoProduto: fotoProdFinal }; const r = await fetch(API_NFC, { method: "POST", body: JSON.stringify(p) }); const res = await r.json(); if(res.sucesso) { window.addLog(`✅ Mágica Atualizada!`, "success"); window.$('modal-editar-ima').style.display = 'none'; window.carregarImas(); } else throw new Error("Erro API."); } catch (e) { window.addLog(`❌ Erro Update: ${e.message}`, "error"); } finally { b.disabled = false; b.innerText = "💾 Atualizar Mágica"; } };
window.confirmarExclusaoIma = function() { const l = parseInt(window.$('edit-ima-linha').value); const i = window.arrayImas.find(x => x.linha === l); if(!i) return; window.$('nome-ima-excluir').innerText = i.nomes; window.$('modal-confirmar-exclusao-ima').style.display = 'flex'; window.$('btn-executar-exclusao-ima').onclick = function() { window.$('modal-confirmar-exclusao-ima').style.display = 'none'; window.excluirRegistroLocal("API_Ignora_Isso", l, "btn-excluir-ima-modal", () => window.$('modal-editar-ima').style.display='none', window.carregarImas, API_NFC); }; };
...
window.carregarAnalytics = async function(forcar=false){ if(window.dadosTotaisAcessos.length>0&&!forcar)return; window.$('loading-analytics').style.display='block'; window.$('painel-dashboard').style.display='none'; try{ const r=await fetch(URL_ANALYTICS_CATALOGO+"?acao=analytics"); const res=await r.json(); if(res.sucesso){window.dadosTotaisAcessos=res.dados; window.aplicarFiltros();}else throw new Error(res.erro); }catch(e){window.$('loading-analytics').style.display='none'; window.addLog(`❌ Erro Analytics: ${e.message}`,"error");} };
window.aplicarFiltros = function(){ window.$('loading-analytics').style.display='none'; const mS=window.$('filtro-mes').value, aS=window.$('filtro-ano').value, tS=window.$('filtro-time').value.toLowerCase(), dI=window.$('data-inicio').value, dF=window.$('data-fim').value; const filtrados=window.dadosTotaisAcessos.filter(i=>{ if(!i.data||!i.time)return false; if(tS!=="todos"){let tr=i.time.toLowerCase(); if(tS==="são paulo"&&(tr==="sao paulo"||tr==="são paulo")){}else if(tr!==tS)return false;} const dT=new Date(i.data); if(dI===""&&dF===""){if(aS!=="todos"&&String(dT.getFullYear())!==aS)return false; if(mS!=="todos"&&String(dT.getMonth()+1).padStart(2,'0')!==mS)return false; return true;} if(dI!==""&&dF!==""){const di=new Date(dI),df=new Date(dF); df.setHours(23,59,59); return dT>=di&&dT<=df;} return true; }); window.processarDashboard(filtrados); window.$('painel-dashboard').style.display='block'; };
window.processarDashboard = function(arr){ window.$('dash-total').innerText=arr.length; if(arr.length===0){ window.$('dash-media').innerText=`Média: 0/dia`; window.$('lista-ranking').innerHTML="<p style='text-align:center;color:#999;padding:20px;'>Nenhum acesso.</p>"; window.$('nome-campeao').innerText="-"; window.$('img-campeao').style.display="none"; window.$('dash-campeao').className="valor"; window.$('dash-pico').innerText=`Pico: Nenhum`; if(window.graficoInstancia)window.graficoInstancia.destroy(); return; } const cT={}, cD={}; arr.forEach(l=>{let t=String(l.time).trim(), d=l.data; if(t)cT[t]=(cT[t]||0)+1; if(d)cD[d]=(cD[d]||0)+1; }); const td=Object.keys(cD).length; window.$('dash-media').innerText=`Média: ${td>0?(arr.length/td).toFixed(1):0} acessos/dia`; let mD="", mAd=0; for(const d in cD){if(cD[d]>mAd){mAd=cD[d];mD=d;}} let dL=""; if(mD){const a=mD.split('-'); if(a.length===3)dL=`${a[2]}/${a[1]}`;} window.$('dash-pico').innerText=`Pico: ${mAd} toques (${dL})`; const aR=Object.keys(cT).map(t=>({nome:t,acessos:cT[t]})).sort((a,b)=>b.acessos-a.acessos); const c=aR[0], iC=TIMES_INFO[c.nome.toLowerCase()]||{cor:"var(--brand-dark)",logo:""}; window.$('nome-campeao').innerText=c.nome; const img=window.$('img-campeao'); if(iC.logo){img.src=iC.logo;img.style.display="block";}else img.style.display="none"; window.$('dash-campeao').className="valor "+(iC.classText||""); let hR="", lg=[], dg=[], cg=[]; const maxA=aR[0].acessos; aR.forEach((i)=>{lg.push(i.nome);dg.push(i.acessos);let iT=TIMES_INFO[i.nome.toLowerCase()]||{cor:"var(--primary)",logo:""}; cg.push(iT.cor); let p=Math.round((i.acessos/maxA)*100); hR+=`<div class="ranking-item"><div class="ranking-nome"><img src="${iT.logo}" onerror="this.style.display='none'"><span class="${iT.classText||''}">${i.nome}</span></div><div class="progresso-bg"><div class="progresso-bar" style="width:${p}%; background-color:${iT.cor};"></div></div><div class="ranking-valor" style="color:${iT.cor};">${i.acessos}</div></div>`;}); window.$('lista-ranking').innerHTML=hR; window.renderizarGrafico(lg,dg,cg); };
...
window.renderListaAnuncios = function(aA){ const lE=window.$('lista-anuncios-admin'); if(aA.length===0){lE.innerHTML="<p style='text-align:center;color:#999;padding:20px;'>Nenhum anúncio.</p>";return;} let h=""; aA.forEach((a)=>{let sB=""; if(a.status==="ATIVO")sB=`<span class="tag-ativo">Ativo</span>`;else if(a.status==="CANCELADO")sB=`<span class="tag-inativo">Cancelado</span>`;else sB=`<span class="tag-pendente">Pendente</span>`; let vT=a.vencimento?`Vence em: ${a.vencimento.split('-').reverse().join('/')}`:"Sem vencimento"; h+=`<div class="list-item"><div class="list-info"><h4 class="list-nome">${a.empresa}</h4><div class="list-detalhes"><span style="color:var(--primary);font-weight:800;">${formatarPreco(a.valor_negociado)||"R$ 0,00"}</span> • ${vT} </div><div style="margin-top:8px;">${sB}</div></div><a href="${a.zap_link}" target="_blank" class="btn-zap-lista">💬</a><button class="btn-editar" onclick="abrirModalEditarAnuncio(${a.linha})">✏️</button></div>`;}); lE.innerHTML=h; };
...
window.renderListaCatalogo = function(aP){ const lE=window.$('lista-produtos-admin'); if(aP.length===0){lE.innerHTML="<p style='text-align:center;color:#999;padding:20px;'>Nenhum produto.</p>";return;} let h=""; aP.forEach(p=>{const fP=p.foto.split(',')[0].trim()||'logo.png'; const sB=p.ativo?`<span class="tag-ativo">Ativo</span>`:`<span class="tag-inativo">Inativo</span>`; h+=`<div class="list-item"><img src="${fP}" class="list-img" onerror="this.src='logo.png'"><div class="list-info"><h4 class="list-nome">${p.nome}</h4><div class="list-detalhes"><span style="color:var(--primary);font-weight:800;">${formatarPreco(p.preco)}</span> • ${p.categoria} </div><div style="margin-top:8px;">${sB}</div></div><button class="btn-editar" onclick="abrirModalEditarProduto(${p.linha})">✏️</button></div>`;}); lE.innerHTML=h; };
...
window.carregarLote = function(tipo) {
    ...
    window.$('btn-gerar-pdf-lote').onclick = () => window.gerarPDFLote(tipo);
};
...
window.selecionarClienteLote = function() {
    ...
    filtradosDoCliente.forEach(v => {
        let onClickFn = window.loteAtualTipo === 'Pendente' ? `enviarZapCobranca(${v.linha})` : `enviarZapRecibo(${v.linha})`;
        h += `
        <div class="lote-item">
...
window.aplicarPrecoSugerido = function() {
    ...
    if (!isNaN(val)) {
        window.$('venda-valor').value = window.fmt(val);
    }
};
...
window.renderizarListaVendas = function(aV){ const lD=window.$('lista-vendas'); lD.innerHTML=""; let sP=0, sPa=0, sL=0, sQ=0; if(aV.length===0){ lD.innerHTML="<p style='text-align:center;color:#999;padding:20px;'>Nenhuma venda.</p>"; window.$('vendas-total-valor').innerText="R$ 0,00";window.$('vendas-pendentes-valor').innerText="R$ 0,00"; window.$('vendas-pagas-valor').innerText="R$ 0,00";window.$('vendas-lucro-valor').innerText="R$ 0,00"; window.$('vendas-qtd-valor').innerText="0 un"; return; } let h=""; aV.forEach(v=>{ let nV=window.limparValorPlanilha(v.valor_venda), nL=window.limparValorPlanilha(v.lucro), nQ=parseFloat(v.qtd)||0, isP=v.status.toLowerCase()==="pendente"; sQ+=nQ; if(isP)sP+=nV;else sPa+=nV; sL+=nL; let cs=isP?"card-pendente":"", em=isP?"🟡":"🟢", lS=v.lucro&&nL>0?`💰 Lucro: ${window.formatarDoBanco(v.lucro)}`:""; let bB = isP ? `<div class="acoes-venda-box"><button class="btn-novera btn-n-cobranca" onclick="solicitarNomeDocumento(${v.linha}, 'Pendente')" title="Gerar Lembrete">🔔</button><button class="btn-novera btn-n-baixa" onclick="abrirConfirmacaoPagamento(${v.linha})" title="Marcar como Pago">💸</button></div>` : `<div class="acoes-venda-box"><button class="btn-novera btn-n-recibo" onclick="solicitarNomeDocumento(${v.linha}, 'Pago')" title="Gerar Recibo">🧾</button></div>`; h+=`<div class="produto-banco-card ${cs}" style="padding:12px; margin-bottom:10px;"><div class="card-row-top"><div class="pb-info"><div style="display:flex;justify-content:space-between;align-items:center;"><h4 class="pb-nome" style="margin:0; font-size:0.9rem;">${v.cliente}</h4><div style="display:flex; gap:4px;"><button class="btn-novera btn-n-editar" style="width:28px; height:28px; font-size:0.8rem;" onclick="abrirModalEditarVenda(${v.linha})" title="Editar">✏️</button><button class="btn-novera btn-n-apagar" style="width:28px; height:28px; font-size:0.8rem;" onclick="abrirConfirmacaoExclusao(${v.linha})" title="Excluir">🗑️</button></div></div><div class="pb-detalhes" style="flex-direction:column;gap:2px;align-items:flex-start;margin-top:4px;"><div style="display:flex;gap:6px;flex-wrap:wrap;"><span style="color:#64748b;">📅 ${v.data}</span><span style="color:#64748b;">🏷️ ${v.produto} (x${v.qtd})</span></div><div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:2px;"><span style="color:#10b981;font-weight:900;">${lS}</span><span style="font-weight:900;font-size:0.7rem;color:${isP?'#b45309':'#059669'};">${em} ${v.status.toUpperCase()}</span></div></div></div><div class="pb-custo" style="background:transparent;border:none;color:var(--brand-dark);font-size:1.1rem;padding:0 0 0 5px;">${window.fmt(nV)}</div></div>${bB}</div>`; }); lD.innerHTML=h; window.$('vendas-total-valor').innerText=window.fmt(sP+sPa); window.$('vendas-pendentes-valor').innerText=window.fmt(sP); window.$('vendas-pagas-valor').innerText=window.fmt(sPa); window.$('vendas-lucro-valor').innerText=window.fmt(sL); window.$('vendas-qtd-valor').innerText=sQ+" un"; };
