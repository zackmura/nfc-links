...
let arrayImas = [], loteAtualTipo = 'Pendente';
...
function addLog(msg, tipo='info'){ 
    $('terminal').innerHTML+=`<div class="log-line"><span class="log-time">[${new Date().toLocaleTimeString('pt-BR')}]</span><span class="log-${tipo}">${msg}</span></div>`; 
    $('terminal').scrollTop=$('terminal').scrollHeight; 
}
function mostrarAlerta(tit, txt, tipo){ 
    $('modal-titulo-alert').innerText=tit; $('modal-texto-alert').innerText=txt; 
    const ic=$('modal-icone-alert'), b=$('btn-modal-ok'); 
...
async function excluirRegistroLocal(aba, l, idBtn, fnFechar, fnRecarregar, apiBase=URL_ANALYTICS_CATALOGO){ 
    const b=$(idBtn); b.disabled=true; b.innerText="⏳ Excluindo..."; 
    try { 
        const r=await fetch(apiBase,{method:"POST",body:JSON.stringify({acao:"excluir_registro",aba:aba,linha:l})}); 
        const res=await r.json(); 
        if(res.sucesso){ addLog(`🗑️ Registro excluído.`, "success"); if(fnFechar)fnFechar(); if(fnRecarregar)fnRecarregar(); } 
        else { throw new Error("Erro exclusão."); } 
    }catch(e){ apiBase===API_PRECIFICACAO ? mostrarAlerta("Erro","Falha: "+e.message,"error") : alert("Falha: "+e.message); 
    }finally{ b.disabled=false; b.innerText="🗑️ Excluir"; } 
}

function renderizarPlataformas() {
    const list = $('lista-plataformas'); if(!list) return;
    ...
    let h = "", opt = "<option value=''>-- Selecione --</option>";
    plats.forEach((p, idx) => {
...
function salvarPlataforma() {
    const nome = $('config-plat-nome').value.trim(); const pct = parseFloat($('config-plat-pct').value) || 0; const fixa = parseFloat($('config-plat-fixa').value) || 0;
    ...
    $('config-plat-nome').value = ""; $('config-plat-pct').value = ""; $('config-plat-fixa').value = ""; renderizarPlataformas();
}
function excluirPlataforma(idx) { let plats = JSON.parse(localStorage.getItem('minimundo_plataformas') || '[]'); plats.splice(idx, 1); localStorage.setItem('minimundo_plataformas', JSON.stringify(plats)); renderizarPlataformas(); }

async function carregarImas() { 
    $('loading-imas').style.display = 'block'; $('lista-imas-admin').innerHTML = ''; 
    try { const r = await fetch(API_NFC + "?acao=listar"); const res = await r.json(); if (res.sucesso) { arrayImas = res.imas.reverse(); renderListaImas(arrayImas); } else { throw new Error("Erro ao buscar imas."); } 
    } catch (e) { addLog(`Erro Ímãs: ${e.message}`, "error"); } finally { $('loading-imas').style.display = 'none'; } 
}
function renderListaImas(arr) { 
    ...
    arr.forEach(i => { 
        ...
        h += `<div class="list-item"><img src="${fotoUrl}" class="list-img" onerror="this.src='logo.png'"><div class="list-info"><h4 class="list-nome">ID: ${i.id} - ${i.nomes}</h4><div class="list-detalhes">🗓️ Liberação: ${dataFormatada}</div><div style="margin-top:8px;">${statusVisual}</div></div><div style="display:flex; gap:8px;"><button class="btn-novera" style="background:#e0f2fe; border-color:#bae6fd; color:#0369a1; width:auto; padding:0 12px; font-weight:bold; font-size:0.8rem;" onclick="copiarLinkNfc('${linkNfc}')" title="Copiar Link">🔗 Link</button><button class="btn-novera btn-n-editar" onclick="abrirModalEditarIma(${i.linha})" title="Editar Ímã">✏️</button></div></div>`; 
    }); lE.innerHTML = h; 
}
function copiarLinkNfc(link) {
    if (navigator.clipboard && window.isSecureContext) { navigator.clipboard.writeText(link).then(() => { mostrarAlerta("Copiado!", "O link do ímã foi copiado!", "success"); }).catch(() => fallbackCopyTextToClipboard(link));
...
function fallbackCopyTextToClipboard(text) {
    ...
    try { var successful = document.execCommand('copy'); if(successful) { mostrarAlerta("Copiado!", "O link foi copiado para a área de transferência!", "success"); } else { alert("Copie manualmente: " + text); } } catch (err) { alert("Copie manualmente: " + text); } document.body.removeChild(textArea);
}
...
async function fazerUploadRedundante(fC, ik) { 
    ...
    try { const fd = new FormData(); fd.append("imagem", fC); const rp = await fetch(API_ONIONSYS, {method:"POST", headers:{"Authorization":`Bearer ${TOKEN_ONIONSYS}`, "x-tenant-id":"MiniMundo"}, body:fd}); const tx = await rp.text(); if (rp.ok) { const rs = JSON.parse(tx); urlOnion = (rs.arquivos && rs.arquivos.length > 0) ? rs.arquivos[0].url : (rs.url || rs.link || rs.URL || (rs.filename ? `https://api.onionsys.com.br/arquivos/catalogo/${rs.filename}` : "")); addLog("✅ Imagem salva no servidor Primário.", "success"); } } catch (e) { addLog("⚠️ Servidor primário falhou.", "warn"); } 
    if (ik) { try { const fd2 = new FormData(); fd2.append("image", fC); const r2 = await fetch(`https://api.imgbb.com/1/upload?key=${ik}`, {method:"POST", body:fd2}); const d2 = await r2.json(); if (d2.success) { urlImgBB = d2.data.url; addLog("✅ Imagem salva no Backup (ImgBB).", "success"); } } catch (e) { addLog("⚠️ Servidor de backup falhou.", "warn"); } } 
...
async function cadastrarIma() { 
    const id = $('ima-id').value.trim(), nomes = $('ima-nomes').value.trim(), fFoto = $('ima-foto').files, video = $('ima-video').value.trim(), dataOriginal = $('ima-data').value, ik = $('imgbb-key').value.trim(), inputProd = document.getElementById("ima-foto-produto"); 
    ...
    const b = $('btn-salvar-ima'); b.disabled = true; b.innerText = "⏳ Gerando..."; 
    try { 
        addLog(`Subindo foto do cliente...`, "info"); const urlsCliente = await fazerUploadRedundante(await comprimirImagem(fFoto[0], 600, 600, 0.8), ik); 
        addLog(`Subindo foto do produto físico...`, "info"); const urlsProduto = await fazerUploadRedundante(await comprimirImagem(inputProd.files[0], 600, 600, 0.8), ik); 
        ...
        const r = await fetch(API_NFC, { method: "POST", body: JSON.stringify(p) }); const res = await r.json(); 
        if(res.sucesso) { mostrarAlerta("Sucesso!", "Ímã cadastrado com fotos personalizadas!", "success"); carregarImas(); } 
    } catch (e) { addLog(`❌ ERRO: ${e.message}`, "error"); } finally { b.disabled = false; b.innerText = "✨ Gerar Ímã"; } 
}
function abrirModalEditarIma(l) { 
    const i = arrayImas.find(x => x.linha === l); if(!i) return; 
    $('edit-ima-linha').value = i.linha; $('edit-ima-nomes').value = i.nomes; $('edit-ima-video').value = i.linkVideoYoutube; $('edit-ima-foto-antiga').value = i.foto; $('edit-ima-img-preview').src = i.foto ? i.foto.split(',')[0].trim() : "logo.png"; $('edit-ima-foto').value = ""; $('edit-ima-foto-prod-antiga').value = i.fotoProduto || ""; 
    if(i.fotoProduto) { $('edit-ima-prod-preview').src = i.fotoProduto.split(',')[0].trim(); $('edit-ima-prod-preview').style.display = "block"; } else { $('edit-ima-prod-preview').style.display = "none"; } 
    $('edit-ima-foto-prod').value = ""; if(i.dataLiberacao) { $('edit-ima-data').value = String(i.dataLiberacao).substring(0, 16); } else { $('edit-ima-data').value = ""; } 
    $('modal-editar-ima').style.display = 'flex'; 
}
async function salvarEdicaoIma() { 
    const b = $('btn-salvar-edicao-ima'); b.disabled = true; b.innerText = "⏳ Atualizando..."; 
    try { 
        const ik = $('imgbb-key').value.trim(); const idAtual = arrayImas.find(x => x.linha == parseInt($('edit-ima-linha').value)).id; 
        let fotoFinal = $('edit-ima-foto-antiga').value; const novaFoto = $('edit-ima-foto').files; let fotoProdFinal = $('edit-ima-foto-prod-antiga').value; const novaFotoProd = $('edit-ima-foto-prod').files; 
        if(novaFoto.length > 0) { addLog(`Atualizando foto do cliente...`, "info"); fotoFinal = await fazerUploadRedundante(await comprimirImagem(novaFoto[0], 600, 600, 0.8), ik); } 
        if(novaFotoProd.length > 0) { addLog(`Atualizando foto do produto...`, "info"); fotoProdFinal = await fazerUploadRedundante(await comprimirImagem(novaFotoProd[0], 600, 600, 0.8), ik); } 
        let dataLiberacaoStr = ""; if($('edit-ima-data').value) { dataLiberacaoStr = $('edit-ima-data').value + ":00"; } 
        const p = { acao: "atualizar_ima", linha: $('edit-ima-linha').value, id: idAtual, nomes: $('edit-ima-nomes').value, foto: fotoFinal, linkVideoYoutube: $('edit-ima-video').value, dataLiberacao: dataLiberacaoStr, fotoProduto: fotoProdFinal }; 
        const r = await fetch(API_NFC, { method: "POST", body: JSON.stringify(p) }); const res = await r.json(); 
        if(res.sucesso) { addLog(`✅ Mágica Atualizada!`, "success"); $('modal-editar-ima').style.display = 'none'; carregarImas(); } else { throw new Error("Erro API."); }
    } catch (e) { addLog(`❌ Erro Update: ${e.message}`, "error"); } finally { b.disabled = false; b.innerText = "💾 Atualizar Mágica"; } 
}
...
function limparDatasManuais(){ $('data-inicio').value=""; $('data-fim').value=""; } 
function limparFiltrosRapidos(){ $('filtro-mes').value="todos"; $('filtro-ano').value="todos"; }
async function carregarAnalytics(forcar=false){ 
    if(dadosTotaisAcessos.length>0&&!forcar)return; 
    $('loading-analytics').style.display='block'; $('painel-dashboard').style.display='none'; 
    try{ const r=await fetch(URL_ANALYTICS_CATALOGO+"?acao=analytics"); const res=await r.json(); if(res.sucesso){dadosTotaisAcessos=res.dados; aplicarFiltros();}else throw new Error(res.erro); }catch(e){$('loading-analytics').style.display='none'; addLog(`❌ Erro Analytics: ${e.message}`,"error");} 
}
function aplicarFiltros(){ 
    $('loading-analytics').style.display='none'; const mS=$('filtro-mes').value, aS=$('filtro-ano').value, tS=$('filtro-time').value.toLowerCase(), dI=$('data-inicio').value, dF=$('data-fim').value; 
    const filtrados=dadosTotaisAcessos.filter(i=>{ if(!i.data||!i.time)return false; if(tS!=="todos"){let tr=i.time.toLowerCase(); if(tS==="são paulo"&&(tr==="sao paulo"||tr==="são paulo")){}else if(tr!==tS)return false;} const dT=new Date(i.data); if(dI===""&&dF===""){if(aS!=="todos"&&String(dT.getFullYear())!==aS)return false; if(mS!=="todos"&&String(dT.getMonth()+1).padStart(2,'0')!==mS)return false; return true;} if(dI!==""&&dF!==""){const di=new Date(dI),df=new Date(dF); df.setHours(23,59,59); return dT>=di&&dT<=df;} return true; }); 
    processarDashboard(filtrados); $('painel-dashboard').style.display='block'; 
}
function processarDashboard(arr){ 
    $('dash-total').innerText=arr.length; 
    if(arr.length===0){ $('dash-media').innerText=`Média: 0/dia`; $('lista-ranking').innerHTML="<p style='text-align:center;color:#999;padding:20px;'>Nenhum acesso.</p>"; $('nome-campeao').innerText="-"; $('img-campeao').style.display="none"; $('dash-campeao').className="valor"; $('dash-pico').innerText=`Pico: Nenhum`; if(graficoInstancia)graficoInstancia.destroy(); return; } 
    const cT={}, cD={}; arr.forEach(l=>{let t=String(l.time).trim(), d=l.data; if(t)cT[t]=(cT[t]||0)+1; if(d)cD[d]=(cD[d]||0)+1; }); const td=Object.keys(cD).length; $('dash-media').innerText=`Média: ${td>0?(arr.length/td).toFixed(1):0} acessos/dia`; let mD="", mAd=0; for(const d in cD){if(cD[d]>mAd){mAd=cD[d];mD=d;}} let dL=""; if(mD){const a=mD.split('-'); if(a.length===3)dL=`${a[2]}/${a[1]}`;} $('dash-pico').innerText=`Pico: ${mAd} toques (${dL})`; const aR=Object.keys(cT).map(t=>({nome:t,acessos:cT[t]})).sort((a,b)=>b.acessos-a.acessos); const c=aR[0], iC=TIMES_INFO[c.nome.toLowerCase()]||{cor:"var(--brand-dark)",logo:""}; $('nome-campeao').innerText=c.nome; const img=$('img-campeao'); if(iC.logo){img.src=iC.logo;img.style.display="block";}else img.style.display="none"; $('dash-campeao').className="valor "+(iC.classText||""); let hR="", lg=[], dg=[], cg=[]; const maxA=aR[0].acessos; aR.forEach((i)=>{lg.push(i.nome);dg.push(i.acessos);let iT=TIMES_INFO[i.nome.toLowerCase()]||{cor:"var(--primary)",logo:""}; cg.push(iT.cor); let p=Math.round((i.acessos/maxA)*100); hR+=`<div class="ranking-item"><div class="ranking-nome"><img src="${iT.logo}" onerror="this.style.display='none'"><span class="${iT.classText||''}">${i.nome}</span></div><div class="progresso-bg"><div class="progresso-bar" style="width:${p}%; background-color:${iT.cor};"></div></div><div class="ranking-valor" style="color:${iT.cor};">${i.acessos}</div></div>`;}); $('lista-ranking').innerHTML=hR; renderizarGrafico(lg,dg,cg); 
}
    ...
    aA.forEach((a)=>{
        ...
        <div class="list-item">
            <div class="list-info">
                ...
                <div class="list-detalhes"><span style="color:var(--primary);font-weight:800;">${formatarPreco(a.valor_negociado)||"R$ 0,00"}</span> • ${vT} </div>
                <div style="margin-top:8px;">${sB}</div>
            </div>
            <a href="${a.zap_link}" target="_blank" class="btn-zap-lista">💬</a>
            <button class="btn-editar" onclick="abrirModalEditarAnuncio(${a.linha})">✏️</button>
        </div>`;
    }); 
    lE.innerHTML=h; 
}
...
function renderListaCatalogo(aP){ 
    const lE=$('lista-produtos-admin'); 
    ...
    aP.forEach(p=>{
        ...
        <div class="list-item">
            ...
            <div class="list-info">
                ...
                <div class="list-detalhes"><span style="color:var(--primary);font-weight:800;">${formatarPreco(p.preco)}</span> • ${p.categoria} </div>
                <div style="margin-top:8px;">${sB}</div>
            </div>
            <div style="display:flex; gap:6px; flex-shrink:0;">
                <button class="btn-novera btn-n-editar" onclick="abrirModalEditarProduto(${p.linha})" title="Editar Produto">✏️</button>
            </div>
        </div>`;
    }); 
    lE.innerHTML=h; 
...
function fileToBase64(f){return new Promise((r,j)=>{const rd=new FileReader();rd.readAsDataURL(f);rd.onload=()=>r(rd.result.split(',')[1]);rd.onerror=e=>j(e);});}
...
function gerarPDFLote() {
    ...
    gerarHTMLImpressao(grupos, loteAtualTipo);
}
...
function aplicarPrecoSugerido() {
    ...
    if (!isNaN(val)) { $('venda-valor').value = fmt(val); }
}
...
function gerarRelatorioCliente() { const busca = $('busca-venda').value.trim().toLowerCase(); if(!busca) return mostrarAlerta("Atenção", "Para gerar o extrato, digite o nome do cliente na barra de busca primeiro!", "warning"); const norm = s => s.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase(); const tB = norm(busca); const mS = $('filtro-v-mes').value, aS = $('filtro-v-ano').value, stS = $('filtro-v-status').value.toLowerCase(); const filtrados = arrayVendas.filter(v => { if (!norm(v.cliente).includes(tB)) return false; if (stS !== "todos" && v.status.toLowerCase() !== stS) return false; if (mS !== "todos" || aS !== "todos") { let pt = v.data.includes('/') ? v.data.split('/') : (v.data.includes('-') ? v.data.split('-').reverse() : []); if (pt.length >= 3) { let m = String(pt[1]).padStart(2,'0'), a = String(pt[2]).substring(0,4); if (mS !== "todos" && m !== mS) return false; if (aS !== "todos" && a !== aS) return false; } else return false; } return true; }); if(filtrados.length === 0) return mostrarAlerta("Ops", "Nenhuma venda encontrada para este cliente nos filtros atuais.", "warning"); let total = 0; let texto = `*🧾 EXTRATO DE PEDIDOS | MINI MUNDO 3D*\n*Cliente:* ${filtrados[0].cliente}\n\n`; filtrados.forEach(v => { let val = limparValorPlanilha(v.valor_venda); total += val; let icone = v.status.toLowerCase() === 'pago' ? '🟢' : '🟡'; texto += `📅 *${v.data}* - ${v.produto} (x${v.qtd})\n💰 Valor: ${fmt(val)} | Status: ${icone} ${v.status}\n\n`; }); texto += `*TOTAL DA FATURA: ${fmt(total)}*`; navigator.clipboard.writeText(texto).then(() => { mostrarAlerta("Copiado!", "O relatório do cliente foi copiado. É só colar no WhatsApp!", "success"); }).catch(() => { alert("Erro ao copiar. Segue o texto:\n\n" + texto); }); }
function filtrarVendas(){ const norm = s => s.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase(); const mS=$('filtro-v-mes').value, aS=$('filtro-v-ano').value, stS=$('filtro-v-status').value.toLowerCase(), tB=norm($('busca-venda').value); const flt=arrayVendas.filter(v=>{ if(tB&&!(norm(v.cliente).includes(tB)||norm(v.produto).includes(tB)))return false; if(stS!=="todos"&&v.status.toLowerCase()!==stS)return false; if(mS!=="todos"||aS!=="todos"){ let pt=[]; if(v.data.includes('/'))pt=v.data.split('/'); else if(v.data.includes('-')){let p=v.data.split('-');pt=[p[2],p[1],p[0]];} else if(v.data.includes(' ')){let td=new Date(v.data);if(!isNaN(td))pt=[td.getDate(),String(td.getMonth()+1).padStart(2,'0'),td.getFullYear()];} if(pt.length>=3){let m=String(pt[1]).padStart(2,'0'), a=String(pt[2]).substring(0,4); if(mS!=="todos"&&m!==mS)return false; if(aS!=="todos"&&a!==aS)return false;}else return false; } return true; }); vendasFiltradasAtuais = flt; renderizarListaVendas(flt); }
    ...
    aV.forEach(v => { 
        ...
        <div class="produto-banco-card ${cs}" style="padding:15px; margin-bottom:12px;">
            ...
            <div class="acoes-venda-box" style="margin-top: 15px; border-top: 1px dashed var(--border-color); padding-top: 12px; display: flex; justify-content: flex-end; gap: 8px;">
                ${bB}
                <button class="btn-novera btn-n-editar" onclick="abrirModalEditarVenda(${v.linha})" title="Editar Venda">✏️</button>
                <button class="btn-novera btn-n-apagar" onclick="abrirConfirmacaoExclusao(${v.linha})" title="Excluir Venda">🗑️</button>
            </div>
        </div>`; 
    }); 
    lD.innerHTML = h; 
