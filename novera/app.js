...
// --- MÁSCARA DE DINHEIRO GLOBAL (COM GATILHO DE CÁLCULO) ---
document.addEventListener('input', function (e) {
    if (e.target.classList.contains('mask-money')) {
        ...
        // Gatilhos para recalcular totais automaticamente ao digitar o unitário
...
// --- UI E CARREGAMENTO ---
function mostrarLoading(m) { const l = document.getElementById("global-loader"); l.querySelector("p").innerText = m; l.style.display = "flex"; }
function ocultarLoading() { document.getElementById("global-loader").style.display = "none"; }
...
// --- RÓTULOS (ORDEM DO CÓDIGO INVERSA N003, N002...) ---
...
function renderizarOpcoesPrecificacao() { 
    ...
    rotulosGlobal.sort((a,b) => String(b.codigo || "").localeCompare(String(a.codigo || ""))).forEach(r => { 
        htmlSelect += `<option value="${r.codigo}|${r.essencia}">${r.codigo} - ${r.essencia}</option>`; 
    }); 
    select.innerHTML = htmlSelect; 
}

async function salvarRotulo() { const essencia = padronizarTexto(document.getElementById('r-essencia').value); const codForn = padronizarTexto(document.getElementById('r-codigo-forn').value); const marca = padronizarTexto(document.getElementById('r-marca').value); if (!essencia) return mostrarAlerta("Atenção", "Preencha a Essência.", "warning"); mostrarLoading("Salvando Rótulo..."); try { const res = await fetch(API_NOVERA, { method: "POST", body: JSON.stringify({ acao: "salvar_rotulo", essencia: essencia, codigo_forn: codForn, marca: marca }) }); const resultado = await res.json(); if (resultado.sucesso) { mostrarAlerta("Criado!", `Identidade gerada.`, "success"); document.getElementById('r-essencia').value = ""; sincronizarDadosUnico(); } else { mostrarAlerta("Erro", resultado.erro === "DUPLICADO_ROTULO" ? "Essência já cadastrada." : resultado.erro, "error"); } } catch (e) { mostrarAlerta("Erro", "Falha de conexão.", "error"); } finally { ocultarLoading(); } }
function abrirModalEditarRotulo(linha) { const r = rotulosGlobal.find(x => x.linha === linha); if (!r) return; document.getElementById('edit-r-linha').value = r.linha; document.getElementById('edit-r-essencia').value = r.essencia; document.getElementById('edit-r-codigo-forn').value = r.codigo_forn; document.getElementById('edit-r-marca').value = r.marca; document.getElementById('modal-editar-rotulo').style.display = 'flex'; }
async function salvarEdicaoRotulo() { mostrarLoading("Atualizando..."); try { const py = { acao: "atualizar_rotulo", linha: document.getElementById('edit-r-linha').value, essencia: padronizarTexto(document.getElementById('edit-r-essencia').value), codigo_forn: padronizarTexto(document.getElementById('edit-r-codigo-forn').value), marca: padronizarTexto(document.getElementById('edit-r-marca').value) }; const res = await fetch(API_NOVERA, { method: "POST", body: JSON.stringify(py) }); const json = await res.json(); if(json.sucesso) { document.getElementById('modal-editar-rotulo').style.display = 'none'; mostrarAlerta("Atualizado!", "Edição salva.", "success"); sincronizarDadosUnico(); } } catch(e) { mostrarAlerta("Erro", "Falha de conexão.", "error"); } finally { ocultarLoading(); } }
...
function renderizarEstoque() { 
    ...
    // ORDENANDO: Tem estoque em cima, sem estoque embaixo, mas ORDENADO PELO CÓDIGO INVERTIDO
    filtrados.sort((a, b) => { 
...
async function salvarEdicaoEstoque() { 
    const fileInput = document.getElementById('edit-e-foto-nova'); 
    let fotoFinal = document.getElementById('edit-e-foto-antiga').value; 
    const escolha = document.getElementById('escolha-foto-ia').value; 
    if (escolha === 'ia') { 
        mostrarLoading("Enviando Imagem da IA..."); 
        try { const imgSrc = document.getElementById('ai-img-result').src; const response = await fetch(imgSrc); const blob = await response.blob(); const file = new File([blob], "produto_ia.jpg", { type: "image/jpeg" }); if(!KEY_IMGBB && !TOKEN_ONIONSYS) throw new Error("Chaves ausentes."); fotoFinal = await uploadDuplo(file); } catch(e) { ocultarLoading(); return mostrarAlerta("Erro", "Falha ao salvar foto da IA. Faltam chaves na Configuração?", "error"); } 
    } else if (fileInput.files.length > 0 && escolha === 'original') { 
        mostrarLoading("Enviando Nova Imagem..."); 
        try { const blob = await comprimirImagem(fileInput.files[0], 800, 800, 0.8); if(!KEY_IMGBB && !TOKEN_ONIONSYS) throw new Error("Chaves ausentes."); fotoFinal = await uploadDuplo(blob); } catch (err) { ocultarLoading(); return mostrarAlerta("Erro na Imagem", err.message, "error"); } 
    } 
    const linha = document.getElementById('edit-e-linha').value; const q = document.getElementById('edit-e-qtd').value; const c = parseDinheiro(document.getElementById('edit-e-custo').value); const p = parseDinheiro(document.getElementById('edit-e-preco').value); const cod = document.getElementById('edit-e-codigo').value; 
    document.getElementById('modal-editar-estoque').style.display = 'none'; 
    mostrarLoading("Salvando...");
    try { const py = { acao: "atualizar_estoque", linha: linha, nome: document.getElementById('edit-e-nome').value, tipo: document.getElementById('edit-e-tipo').value, qtd: q, custo: fmtPlanilha(c), preco: fmtPlanilha(p), foto: fotoFinal, codigo: cod }; fetch(API_NOVERA, { method: "POST", body: JSON.stringify(py) }).then(() => { mostrarAlerta("Atualizado!", "Estoque ajustado.", "success"); sincronizarDadosUnico(); }); } catch(e) {} 
}
...
function salvarCompra() { 
    const data = document.getElementById('c-data').value; const cat = padronizarTexto(document.getElementById('c-categoria').value); const item = padronizarTexto(document.getElementById('c-item').value); const qtd = document.getElementById('c-qtd').value; const val = document.getElementById('c-valor').value; 
...
function confirmarCompraModal() { 
    const linha = document.getElementById('mc-linha').value; const local = padronizarTexto(document.getElementById('mc-local').value); const socio = padronizarTexto(document.getElementById('mc-socio').value); const valorUnit = document.getElementById('mc-valor-unit').value; const total = document.getElementById('mc-total').value; 
    const c = comprasGlobal.find(x => x.linha == linha);
    document.getElementById('modal-comprar-item').style.display = 'none'; 
    mostrarLoading("Lançando Despesa..."); 
    ...
    .then(() => { mostrarAlerta("Pronto!", "Baixa dada e Gasto registrado.", "success"); sincronizarDadosUnico(); }); 
}

function abrirModalEditarCompra(linha) { 
    const c = comprasGlobal.find(x => x.linha === linha); if (!c) return; 
    document.getElementById('edit-c-linha').value = c.linha; document.getElementById('edit-c-item').value = c.item; document.getElementById('edit-c-qtd').value = c.qtd; document.getElementById('edit-c-valor').value = safeFmt(c.valor_previsto); document.getElementById('edit-c-data').value = c.dataPrevista; document.getElementById('edit-c-categoria').value = c.categoria; document.getElementById('modal-editar-compra').style.display = 'flex'; 
}

function salvarEdicaoCompra() { 
    const linha = document.getElementById('edit-c-linha').value; const item = padronizarTexto(document.getElementById('edit-c-item').value); const qtd = document.getElementById('edit-c-qtd').value; const valor = document.getElementById('edit-c-valor').value; const data = document.getElementById('edit-c-data').value; const cat = padronizarTexto(document.getElementById('edit-c-categoria').value); 
    ...
    fetch(API_NOVERA, { method: "POST", body: JSON.stringify(py) }).then(() => { 
        fetch(API_NOVERA, { method: "POST", body: JSON.stringify({ acao: "salvar_compra", data: data, categoria: cat, item: item, qtd: qtd, valor: fmtPlanilha(parseDinheiro(valor)) }) })
        .then(() => { mostrarAlerta("Atualizado!", "Planejamento alterado.", "success"); sincronizarDadosUnico(); }); 
    }); 
}
...
async function confirmarCompraLoteModal() { 
    const checkboxes = document.querySelectorAll('.chk-item-compra-lote:checked'); const local = padronizarTexto(document.getElementById('mcl-local').value); const socio = padronizarTexto(document.getElementById('mcl-socio').value); 
...
function excluirComprasEmLote() { 
    ...
    abrirConfirmacao("Excluir Selecionados?", `Deseja apagar os ${checkboxes.length} itens marcados?`, "🗑️", "#A05252", "#803f3f", "🗑️ Confirmar", async () => { 
        ...
        for (let chk of checkboxes) { await fetch(API_NOVERA, { method: "POST", body: JSON.stringify({ acao: "excluir_registro", aba: "Compras", linha: chk.value }) }); } 
        mostrarAlerta("Removidos!", "Itens apagados.", "success"); 
        sincronizarDadosUnico(); 
    }); 
}

// CÁLCULO GASTOS CORRIGIDO PARA USAR EVENTOS DE MÁSCARA
...
function salvarGasto() { const data = document.getElementById('g-data').value, local = padronizarTexto(document.getElementById('g-local').value), socio = padronizarTexto(document.getElementById('g-socio').value), item = padronizarTexto(document.getElementById('g-item').value), qtd = document.getElementById('g-qtd').value, valor = parseDinheiro(document.getElementById('g-valor').value), total = parseDinheiro(document.getElementById('g-total').value); if(!data || !item || valor === 0) return mostrarAlerta("Atenção", "Preencha a Data, Item e Valor.", "warning"); mostrarLoading("Salvando..."); fetch(API_NOVERA, { method: "POST", body: JSON.stringify({ acao: "salvar_gasto", data: data, local: local, socio: socio, item: item, qtd: qtd, valor: fmtPlanilha(valor), total: fmtPlanilha(total) }) }).then(() => { mostrarAlerta("Sucesso", "Despesa lançada.", "success"); document.getElementById('g-item').value = ""; document.getElementById('g-valor').value = ""; document.getElementById('g-total').value = ""; sincronizarDadosUnico(); }); }
...
function salvarEdicaoGasto() { 
    const py = { 
        ...
        linha: document.getElementById('edit-g-linha').value, 
        data: document.getElementById('edit-g-data').value, 
        local: padronizarTexto(document.getElementById('edit-g-local').value), 
        socio: padronizarTexto(document.getElementById('edit-g-socio').value), 
        item: padronizarTexto(document.getElementById('edit-g-item').value), 
        qtd: document.getElementById('edit-g-qtd').value, 
        valor: fmtPlanilha(parseDinheiro(document.getElementById('edit-g-valor').value)), 
        total: fmtPlanilha(parseDinheiro(document.getElementById('edit-g-total').value)) 
    }; 
    document.getElementById('modal-editar-gasto').style.display = 'none'; 
    mostrarLoading("Salvando..."); 
    fetch(API_NOVERA, { method: "POST", body: JSON.stringify(py) }).then(() => { mostrarAlerta("Atualizado!", "Edição salva.", "success"); sincronizarDadosUnico(); }); 
}
...
function salvarEncomenda() { const data = document.getElementById('e-data').value, cli = padronizarTexto(document.getElementById('e-cliente').value), item = padronizarTexto(document.getElementById('e-item').value), qtd = document.getElementById('e-qtd').value, obs = document.getElementById('e-obs').value; if(!data || !cli || !item) return mostrarAlerta("Atenção", "Preencha Data, Cliente e Item.", "warning"); mostrarLoading("Salvando..."); fetch(API_NOVERA, { method: "POST", body: JSON.stringify({ acao: "salvar_encomenda", data: data, cliente: cli, item: item, qtd: qtd, status: 'Pendente', obs: obs }) }).then(() => { mostrarAlerta("Registrado", "Encomenda salva.", "success"); document.getElementById('e-item').value = ""; document.getElementById('e-obs').value = ""; sincronizarDadosUnico(); }); }
function mudarStatusEncomenda(linha, novoStatus) { mostrarLoading("Atualizando..."); fetch(API_NOVERA, { method: "POST", body: JSON.stringify({ acao: "atualizar_encomenda_status", linha: linha, status: novoStatus }) }).then(()=>sincronizarDadosUnico()); }
