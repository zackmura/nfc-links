...
async function sincronizarDadosUnico() { 
    ...
    } catch(e) { mostrarAlerta("Falha de Conexão", "Erro ao carregar dados. (" + e.message + ")", "error"); } finally { 
...
// --- RÓTULOS ---
function renderizarRotulos() { const lista = document.getElementById("lista-rotulos-cadastrados"); if (rotulosGlobal.length === 0) { lista.innerHTML = "<p style='text-align:center; color:#999; font-size:0.8rem;'>Vazio.</p>"; return; } let htmlLista = ""; rotulosGlobal.sort((a,b) => a.essencia.localeCompare(b.essencia)).forEach(r => { htmlLista += `<div class="rotulo-card"><div class="rotulo-info"><h4>${r.essencia} <span style="font-size:0.7rem; color:#999; font-weight:500;">(${r.marca})</span></h4><p>Cód Forn: <b>${r.codigo_forn || '-'}</b></p></div><div style="display:flex; gap:12px; align-items:center;"><div class="rotulo-badge">${r.codigo}</div><div style="display:flex; gap:5px;"><button class="btn-acao" onclick="abrirModalEditarRotulo(${r.linha})" title="Editar">✏️</button><button class="btn-acao" onclick="prepararExclusaoRegistro('Tabela Rotulo Novera', ${r.linha}, '${r.codigo}')" title="Excluir">🗑️</button></div></div></div>`; }); lista.innerHTML = htmlLista; }
function renderizarOpcoesPrecificacao() { const select = document.getElementById("n-produto"); let htmlSelect = '<option value="">Selecione a Essência Base...</option>'; rotulosGlobal.sort((a,b) => a.essencia.localeCompare(b.essencia)).forEach(r => { htmlSelect += `<option value="${r.codigo}|${r.essencia}">${r.codigo} - ${r.essencia}</option>`; }); select.innerHTML = htmlSelect; }
...
function renderizarVendas() { 
    ...
    estoqueGlobal.sort((a,b)=>a.nome.localeCompare(b.nome)).forEach(e => { let qtdAtual = parseFloat(e.qtd) || 0; if (qtdAtual > 0) { let exibeCodigo = e.codigo ? e.codigo + ' - ' : ''; htmlVendas += `<option value="${e.nome}">${exibeCodigo}${e.nome} (Estoque: ${qtdAtual})</option>`; } }); 
...
function filtrarVendas() { 
    ...
    let filtradas = vendasGlobal.filter(v => { 
        ...
        let dataAlvoIso = "";
        if (fTipoData === 'venda') {
            dataAlvoIso = v.dataVendaIso;
        } else {
            if (v.dataPgtoDisplay) {
                const parts = v.dataPgtoDisplay.split('/');
                if (parts.length === 3) dataAlvoIso = `${parts[2]}-${parts[1]}-${parts[0]}`;
            }
        }
    ...
    if (filtradas.length === 0) { html = "<p style='text-align:center; color:#999; font-size:0.8rem;'>Vazio.</p>"; } else { 
        filtradas.forEach(v => { 
            ...
            const isP = v.status === 'Pago'; if(isP) tRec += val; else tDev += val; 
            ...
            html += `<div class="rotulo-card"><div class="rotulo-info"><h4>${v.cliente} <span class="status-badge ${isP ? 'status-pago' : 'status-pendente'}">${v.status}</span></h4><p style="color:var(--primary); font-weight:800; font-size:0.7rem;">${v.dataVendaDisplay || v.dataVendaIso}</p><p><b>${v.qtd}x</b> ${nomeHtml} • Sócio: ${v.socio}</p><p style="font-size:0.7rem; color:#2e7d32; font-weight:700; margin-top:3px;">Lucro: ${safeFmt(v.lucro)} (Markup: ${v.markup})</p>${textoObservacao}</div><div style="display:flex; flex-direction:column; gap:8px; align-items:flex-end;"><div class="custo-val">${safeFmt(v.valor_venda)}</div><div style="display:flex; gap:5px;">${btnAcaoExtra}${!isP ? `<button class="btn-acao" style="background:#e8f5e9; color:#2e7d32; border-color:#bbf7d0;" onclick="darBaixaVenda(${v.linha})" title="Marcar Pago">💲</button>` : ''}<button class="btn-acao" onclick="abrirModalEditarVenda(${v.linha})" title="Editar">✏️</button><button class="btn-acao" onclick="prepararExclusaoRegistro('Vendas', ${v.linha}, 'Venda de ${v.cliente}')" title="Excluir">🗑️</button></div></div></div>`; 
        }); 
    } 
    ...
    document.getElementById('dash-v-lucro').innerText = fmt(tLuc); 
